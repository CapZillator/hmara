...
async function getGeoCoords(some){
  return some;
}
...
function getImage(code){
  let result;
  switch(code){
    case "01d": result = "01d.svg"; break;
    case "01n": result = "01n.svg"; break;
    case "02d": result = "02d.svg"; break;
    case "02n": result = "02n.svg"; break;
    case "03d": result = "03d.svg"; break;
    case "03n": result = "03d.svg"; break;
    case "04d": result = "04d.svg"; break;
    case "04n": result = "04d.svg"; break;
    case "09d": result = "10d.svg"; break;
    case "09n": result = "10d.svg"; break;
    case "10d": result = "09d.svg"; break;
    case "10n": result = "09d.svg"; break;
    case "11d": result = "11d.svg"; break;
    case "11n": result = "11d.svg"; break;
    case "13d": result = "13d.svg"; break;
    case "13n": result = "13d.svg"; break;
    case "50d": result = "50d.svg"; break;
    case "50n": result = "50d.svg"; break;
    default: result = "03d.svg";
  }
  return `img/${result}`;
}
...
function GetRoundchartUVHover(props){
...
function GetChart(props){//Возвращает инфографику: t, осадки, давление за 8 дней
  ...
  let press_legend = <text x={side_legend_indent} y={y_presstext} className="chart_units_side_press">{props.cur_lang.description.press_name}, {props.cur_lang.pressure}</text>;
...
function GetCurrentWind(props){
  ...
  let wind_speed_units = (units === 'metric') ? cur_lang.wind.units.metric: cur_lang.wind.units.imperial;
  let bofort = getBofortscaleWind(weather.wind_speed, units);
  let result = <div className="cur_wind">
      <h3>{cur_lang.wind.desc.name}</h3>
      <div className="elements"><div><img src="img/wind_dir.svg" className={`wind_dir_${getWindDir(weather.wind_deg)}`}/></div><div>{cur_lang.wind.direction[getWindDir(weather.wind_deg)]}</div><div>{Math.round(weather.wind_speed)} {wind_speed_units}</div></div>
      <p>{cur_lang.wind.desc.bofortname}: <span className="bofort_points" title={cur_lang.wind.desc.points}>{bofort}</span> - {cur_lang.wind.desc.bofort[bofort]}</p>
      </div>;
  return result;
}
...
function GetHourlyForecast(props){//Возвращает почасовой прогноз
  ...
  let result = props.forecast.hourly.map((h, i) => {
      ...
      return <div key={i}>
        {date}<img src={getImage(h.weather[0].icon)}/>
        ...
        <p><img src="img/wind_dir.svg" className={`wind_dir_${getWindDir(h.wind_deg)}`}/></p>
...
function GetDailyForecast(props){//Возвращает прогноз по дням
  ...
  let result = props.forecast.daily.map((d, i) => {
      ...
      let sunchart = <div className="sunchart_wrapper"><GetSunChart sunrise={sunrise} sunset={sunset} day_dur={day_dur} cur_lang={props.cur_lang}/></div>;
      return <div key={i}><h3>{date}</h3>
        <div className="single_forecast_wrapper"><div className="day_sector">
          {morn}{day}{eve}{night}
        </div><div>
          <p><img src={getImage(d.weather[0].icon)}/></p>
          ...
          <img src="img/wind_dir.svg" className={`wind_dir_${getWindDir(d.wind_deg)}`} alt={props.cur_lang.wind.direction[getWindDir(d.wind_deg)]}/>
...
class WeatherScreen extends React.Component {
  ...
  render() {
    ...
    const time_of_day = getTimeOfDay(weather);//Возвращает время суток: светлое или тёмное
    ...
    let cur_wind = <GetCurrentWind weather={weather} cur_lang={cur_lang} units={units}/>;//Текущее направление, скорость ветра
    ...
    let chart = <GetChart data={weather} cur_lang={cur_lang} units={units}/>;//Инфографика t, p, precip на 8 дней
    let forecast = <div>{hourly}</div>;
    let switch_buttons = <div><button onClick={this.onHModeClick} className="switch_active">{cur_lang.hourlyfrc}</button><button onClick={this.onDModeClick}  className="switch_off">{cur_lang.dailyfrc}</button></div>;
    if (this.state.active_mode === "d") {
      forecast = <div className="daily_forecast_wrapper">{daily}</div>;
      switch_buttons = <div><button onClick={this.onHModeClick} className="switch_off">{cur_lang.hourlyfrc}</button><button onClick={this.onDModeClick}  className="switch_active">{cur_lang.dailyfrc}</button></div>;
    };
    ...
    return(
      <div>
          ...
          <aside className={sidebar_class}>
            ...
            <div><select onChange={this.onChangeLang} value={lang_option}>
              {options}
            </select>
            <select onChange={this.onChangeUnits} value={units}>
...
class App extends React.Component {
  ...
  render() {
    let langpack = [{code: 'ru', reqlang: 'ru', name: 'Русский', feels_like: "ощущается как", clouds: "Облачность", wind: {units: {metric: "м/с", imperial: "миля/ч"}, direction: {N: "С", NE: "СВ", E: "В", SE: "ЮВ", S: "Ю", SW: "ЮЗ", W: "З", NW: "СЗ"}, 
                      ...
                      dailyfrc: "Краткий", hourlyfrc: "Подробный прогноз", addtofav: 'Добавлено в избранное', alreadyinfav: 'Уже в избранном',
                    ...
                    {code: 'by', reqlang: 'en', name: 'Беларуская', feels_like: "адчуваецца як", clouds: "Воблачнасць", wind: {units: {metric: "м/с", imperial: "миля/г"}, direction: {N: "Пн", NE: "Пн-У", E: "У", SE: "Пд-У", S: "Пд", SW: "Пд-З", W: "З", NW: "Пн-З"}, 
                      ...
                      dailyfrc: "Кароткі", hourlyfrc: "Падрабязны прагноз", addtofav: 'Дададзена у абранае', alreadyinfav: 'Ужо ў абраным',
                    ...
                    {code: 'ua', reqlang: 'ua', name: 'Українська', feels_like: "відчувається як", clouds: "Хмарність", wind: {units: {metric: "м/с", imperial: "миля/г"}, direction: {N: "Пн", NE: "Пн-Сх", E: "Сх", SE: "Пд-Сх", S: "Пд", SW: "Пд-Зх", W: "Зх", NW: "Пн-Зх"}, 
                      ...
                      dailyfrc: "Короткий", hourlyfrc: "Докладний прогноз", addtofav: 'Додано в обране', alreadyinfav: 'Вже в обраному',
                    ...
                    {code: 'en', reqlang: 'en', name: 'English', feels_like: "feels like", clouds: "Cloudiness", wind: {units: {metric: "m/s", imperial: "mph"}, direction: {N: "N", NE: "NE", E: "E", SE: "SE", S: "S", SW: "SW", W: "W", NW: "NW"}, 
                      ...
                      dailyfrc: "Short", hourlyfrc: "Detailed forecast", addtofav: 'Added to favorites', alreadyinfav: 'Already in favorites', 
